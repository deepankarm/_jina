...
def get_logger():
    logFormatter = logging.Formatter(fmt=' %(name)s :: %(levelname)10s :: %(message)s')
...
class CheckoutBranch(ExitStack):
    ...
    def __exit__(self, *args, **kwargs):
        ...
        # TODO: self.remove_worktree(self._tmpdir)
...
class VersionManagement:
    def __init__(self, args: argparse.Namespace, docsdir: str) -> None:
        ...
        self.last_n_versions = ['v2.5.0', 'v2.4.10', 'v2.4.9']  # 'v2.4.7', 'v2.4.6']
        # self.last_n_versions = VersionManagement.get_last_n_versions(args.repo, args.num_releases)
    ...
    def build_local_only():
        ...
        try:
            f_content = '''import os
os.environ['NUM_RELEASES'] = '1' # special case for 2.5.0, 2.4.10 & 2.4.9
...
'''
    ...
    def build_default_only(self, gh_page_dir):
        VersionManagement.build_versions(
            ...
            gh_page_docs_dir=gh_page_dir,
            build_dir=self.args.build_dir,
        )

    def build_latest_only(self, gh_page_dir):
        VersionManagement.build_versions(
    ...
    def build_non_existing_versions(self, gh_page_dir):
        VersionManagement.build_versions(
    ...
    @staticmethod
    def build_versions(
        versions: Union[str, List[str]], gh_page_docs_dir: str, build_dir: str
    ):
        ...
        for version in versions:
            ...
            with CheckoutBranch(branch_or_tag=version) as tag:
                # Inside {tmpdir}/docs where `version` is checked out.
                VersionManagement.cleanup_version_directory(gh_page_docs_dir, version)
                VersionManagement.build_local_only()
                VersionManagement.recreate_version_directory(
                    gh_page_docs_dir, tag, version, build_dir
                )
    ...
    def move_latest_version_to_root(self):
        shutil.move(
            src=os.path.join(self.docsdir, self.latest_version),
            dst=self.docsdir,
    ...
    def get_versions_status(self):
        ...
        self._non_existing_versions = list(
            set(self.last_n_versions) - set(self._existing_versions)
        )
    ...
    if args.local_only:
        ...
        with gh_page:
            ...
            if args.default_only:
                manager.build_default_only(gh_page_dir=gh_page.docsdir)
                manager.update_dropdown_options(
                    versions=manager.args.default_branch_name
                )
            elif args.latest_only:
                manager.build_latest_only(gh_page_dir=gh_page.docsdir)
                manager.update_dropdown_options(versions=manager.latest_version)
                manager.move_latest_version_to_root()
            else:
                manager.get_versions_status()
                manager.build_non_existing_versions(gh_page_dir=gh_page.docsdir)
                manager.update_dropdown_options(
                    versions=manager.dropdown_versions_in_order
                )
                manager.move_latest_version_to_root()
